...
import { View, Text, StyleSheet, Image, TouchableOpacity, ScrollView, TextInput, KeyboardAvoidingView, Alert } from 'react-native';
...
import cartReducer from '../CartScreen/CartReducer';

const BookDetails = ({ route, navigation }) => {
  const { title, price, details, image } = route.params;
  ...
  const addToCart = () => {
   ...
   dispatch(cartReducer (newItem));
 
    navigation.navigate('Cart', { addedItem: newItem });
  ...
  const scrollToCommentInput = () => {
    if (commentInputRef.current && commentInputRef.current.scrollIntoView) {
      commentInputRef.current.scrollIntoView({ behavior: 'smooth', block: 'nearest', inline: 'start' });
    }
  };
